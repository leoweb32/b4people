...
add_action( 'init', 'ordino_bunch_theme_init');


if( !function_exists( 'ordino_set' ) ) {
	function ordino_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('ordino__font_awesome' ) ) {
	function ordino_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = ordino_set($array, $index )) return $font;
		else return false;
	}
}

if( !function_exists('ordino_load_class' ) ) {
	function ordino_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
	{
...
ordino_load_class( 'enqueue', 'helpers', false );
ordino_load_class( 'bootstrap_walker', 'helpers', false );


if( ordino_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function ordino_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include ORDINO_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include ORDINO_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = ORDINO_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = ORDINO_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( ordino_set( $_GET, 'page' ) == 'ordino'.'_option' ) 
		{
			if( ordino_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'ordino_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = ORDINO_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'ordino'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'ordino'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'ordino' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'ordino' ), // menu label
	));
