...
class Bunch_Enqueue
{
	...
	function bunch_enqueue_scripts()
	{
		...
		$header_style = ordino_set( $options, 'header_style' );
		
 
		$version = $current_theme->get( 'Version' );
		
		$dark_color = ( ordino_set( $options, 'website_theme' ) == 'dark' ) ? true : false;
		
		$dark_color = ( ordino_set( $_GET, 'color_style' ) == 'dark' ) ? true : $dark_color;
		
		if(ordino_set($options, 'color_scheme') == 'color2') : $color = 'css/color2.css'; else: $color = 'css/color1.css'; endif;	
		...
		if( is_single() ) {
			...
			if( $format == 'gallery' ) wp_enqueue_script( array( 'ordino_jquery-flexslider' ) );
			if( $format == 'video' || $format == 'audio' ) wp_enqueue_script( array( 'ordino_jquery-fitvids' ) );
		}
		if( is_singular( 'bunch-projects' ) || $wp_query->is_posts_page || is_search() || is_tag() || is_category() || is_author() || is_archive() ) 
  		wp_enqueue_script( array('ordino_jquery-flexslider', 'ordino_owl.carousel', 'ordino_jquery-fitvids') );
		wp_enqueue_script( array('ordino_custom_script') );
		
		wp_add_inline_style( 'ordino-main-style', $this->wp_head() );
		$opt = _WSH()->option();
		$h_js = 'if( ajaxurl === undefined ) var ajaxurl = "'.esc_url( admin_url( 'admin-ajax.php' ) ).'";';
		wp_add_inline_script( 'ordino-main-script', $h_js );
		
		
	}
	
	function ordino_get_font_settings( $styles )
	{
		$opt = _WSH()->option();
		$protocol = ( is_ssl() ) ? 'https' : 'http';
		$font = array();
		...
		if( ordino_set( $opt, 'use_custom_font' ) ){
			...
			if( $h1 = ordino_set( $opt, 'h1_font_family' ) ) $font[$h1] = urlencode( $h1 ).':400,300,600,700,800';
			if( $h2 = ordino_set( $opt, 'h2_font_family' ) ) $font[$h2] = urlencode( $h2 ).':400,300,600,700,800';
			if( $h3 = ordino_set( $opt, 'h3_font_family' ) ) $font[$h3] = urlencode( $h3 ).':400,300,600,700,800';
		}
		
		if( ordino_set( $opt, 'body_custom_font' ) ){
			if( $body = ordino_set( $opt, 'body_font_family' ) ) $font[$body] = urlencode( $body ).':400,300,600,700,800';
		}
	...
	function wp_head()
	{
		...
		$style .= ordino_set( $opt, 'header_css' );
